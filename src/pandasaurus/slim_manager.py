...
class SlimManager:
    ...
    def get_slim_list(self, ontology: str) -> List[Dict[str, str]]:
        ...
        return [{"name": slim.get_name(), "description": slim.get_description()} for slim in self.slim_list.values()]
    ...
    def get_slim_members(self, slim_name: str) -> List[List[str]]:
        ...
        if slim:
            slim.set_term_list([Term(res.get("term_label"), res.get("term"), True) for res in result])
        else:
            row = next(result)
            slim = Slim(slim_name, row.get("comment"), [Term(row.get("term_label"), row.get("term"), True)])
            slim.set_term_list([Term(res.get("term_label"), res.get("term"), True) for res in result])
        # We probably need a good print method here instead of returning a list of list here.
